...
export async function getAccessToken(code) {
  const client_id = "2384921712f034fd32cf";
  const client_secret = "21a49a7952398d673d4b2aaab4f214eddc66b2f7";
...
export async function fetchUserInfo(accessToken) {
  ...
  const user = {
    ...
    email: userInfo.email,
    bio: userInfo.bio,
    password:"$zq34x6gfr!8r" 
  };
  return user;
}

//function to save or register users with github and google account
export async function authenticateUser(user, router, server) {
  try {
    const response = await fetch(
    ...
    const data = await response.json();
    if (!data) {
      // User not found, handle accordingly (e.g., show a message and redirect)
    ...
    // Handle errors (e.g., show a message and redirect)
    console.error(error);
    secureLocalStorage.setItem("unauthorized_user", user);
