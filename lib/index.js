...
export async function registerUser(formData, setLoading, setError) {
  ...
  try {
    ...
    setLoading(false);
    if (typeof window !== "undefined") {
      window.location.href = window.location.origin ?? "/relevant";
    }
  } catch (error) {
    setLoading(false);
...
export async function handleUserStatusUpdate(id, username, status, setUsers) {
  ...
  if (id) {
    Swal.fire({
      ...
      showCancelButton: true,
      showCloseButton: true,
      footer: "Only update user status with a valid reason!",
      customClass: {
        confirmButton:
          "px-2 py-1 mx-2 bg-red-500 text-white rounded-md hover:text-white hover:bg-red-500 capitalize",
        cancelButton: "px-2 py-1 mx-2 bg-green-500 rounded-md text-white",
...
export async function handleUpdateStatus(status, id, setBlogs) {
  ...
  try {
    const data = await UpdateBlogStatus(id, status);
...
export function createBlog(blogData, setLoading, setError, handleBeforeUnload) {
  Axios.post(`${baseUrl}/blogs`, blogData)
    .then(() => {
      ...
      secureLocalStorage.removeItem("draft_blog_data__");
      revalidateBlogs();
    ...
    .catch((error) => {
      console.error("An error has occurred while publishing:", error);
      setError(error?.response?.data?.error);
      toast.error("Something went wrong");
      setLoading(false);
    });
