...
export default function History() {
  ...
  const clearHistory = () => {
    clearReadHistory({ deleteAll: true });
      ...
      {isLoading ? (
        <div className="grid gap-6">
          {Array.from({ length: 5 }, (_) => (
            <MinimalBlogCardSkeleton key={Math.random()} />
          ))}
        </div>
        ...
        <Card className="py-10">
          ...
          <CardFooter className="text-center flex flex-col items-center justify-center space-y-2">
            ...
            <p className="max-w-sm leading-loose text-xs sm:text-sm mx-auto text-muted-foreground">
