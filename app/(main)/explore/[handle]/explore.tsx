...
import ProfileQRCode from "@/components/custom/profile-qr-code";
...
export default async function ExplorePage({ data }: { data: UserAndBlogs }) {
  ...
  return (
    <section>
      {/* branding div */}
      <div
        className="w-full  lg:min-h-45 min-h-37.5 p-6"
        style={{
          backgroundColor: data.user.branding ?? "#0366F3",
        }}></div>
      <div className="w-full min-h-100 mx-auto px-4 sm:px-6 lg:px-8 py-16 max-w-6xl">
        {/* user card */}
        <div className="px-6 py-4 w-full relative -top-40 rounded-md bg-card shadow border">
          <ProfileQRCode />
          <Avatar
            className="w-30 h-30 relative -top-20 rounded-full m-auto italic"
          ...
          <div className="-mt-20">
            <p className="text-muted-foreground font-semibold  flex items-center justify-center text-lg ">
              <span className="capitalize font-bold text-transparent bg-clip-text bg-linear-to-r from-cyan-500 to-blue-500 via-purple-500 dark:from-cyan-400 dark:to-blue-400">
                {data.user.username}
              </span>
              ...
              {data.user.role === "admin" ? (
                <Image
                  ...
                  width={18}
                  height={18}
                  className="h-auto w-auto max-w-4.5 "
