...
export async function POST(req: NextRequest) {
  try {
	...
	- The tone should be clear, informative, and engaging, depending on the
	topic.
	- Structure the blog post with headings, subheadings, paragraphs, and lists if needed.
	- Do not provide a title heading.
	- For each code response, wrap it in a pre with a class corresponding to its language, for example class=language-javascript,
	class=language-typescript etc. Short codes that need to be highlighted can just be put into <code> tags.
	Example output format:
	<h2>Heading 1</h2>
	<p>Intro paragraph...</p>
	<h3>Subheading</h3>
	<p>More content...</p>
	<pre class="language-javascript"><code>{...code response here}</code></pre>
