...
import type { BlogWithUser, CoverImage } from "@/types";
      ...
      <article className="relative overflow-hidden bg-card  shadow-lg hover:shadow-xl transition-all duration-300 group-hover:brightness-110 filter blog">
        <div className="aspect-video relative">
          ...
          <div className="absolute inset-0 p-8 flex flex-col justify-end text-white">
            <Link
              href={`/read/${post.path}`}
              className="drop-shadow-2xl font-bold text-xl text-shadow-lg line-clamp-2 font-grotesk hover:underline hover:underline-offset-4 hover:text-blue-500 transition-all duration-300">
              {post.title}
            </Link>
