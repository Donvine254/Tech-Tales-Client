...
import type { CommentData, CoverImage, FullBlogData } from "@/types";
...
export default function Slug({ blog, initialComments }: Props) {
  ...
  return (
    <article className="w-full mx-auto m-2 min-h-[75%] px-4 md:px-8 xsm:px-4 max-w-4xl md:mt-4">
      <TooltipProvider>
        <aside>
          <PrismLoader />
        ...
        <section>
          {/* Blog title */}
          <h1 className="text-xl font-grotesk md:text-2xl lg:text-3xl font-bold mb-2">
            {blog.title}
          </h1>
          {/* blog tags */}
          <div className="py-1">
              ...
              <div className="flex space-x-2 items-center flex-wrap">
                {blog.tags.split(",").map((tag: string, index: number) => (
                  <Link
                    ...
                    title={tag}
                    className={`md:px-2 md:py-0 text-blue-500 dark:text-cyan-500 md:bg-transparent md:border border-transparent md:rounded-full uppercase  text-xs  highlight-link-${index}`}>
                    <span>#</span>
                    {tag.trim()}
                  </Link>
                ))}
